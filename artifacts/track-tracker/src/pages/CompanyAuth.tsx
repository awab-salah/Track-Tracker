...
import { companySignIn, companySignUp, sendPasswordReset } from "@/lib/auth";

interface LoginFields { email: string; password: string }
interface RegisterFields { companyName: string; email: string; password: string }
interface ForgotFields { email: string }
...
export default function CompanyAuth() {
  ...
  const onLogin = loginForm.handleSubmit(async (data) => {
    ...
    const result = await companySignIn(data.email, data.password);
  ...
  const onRegister = registerForm.handleSubmit(async (data) => {
    setFormError("");
    setIsSubmitting(true);
    const result = await companySignUp(data.email, data.password, data.companyName);
    setIsSubmitting(false);
    if (result.error) {
      setFormError(result.error);
      return;
    ...
    <MobileLayout>
      <div className="flex flex-col flex-1 p-6 relative z-10">
        ...
        <h2 className="text-3xl font-extrabold text-foreground mb-6">
          {mode === "forgot" ? "استعادة كلمة المرور" : "إدارة شركتك"}
        </h2>
